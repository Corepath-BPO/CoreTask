...
const BY_SUBTYPE: Record<string, ReactNode> = {
  ...
  /* Triggers the engine has no event for, greyed with their reason. */
  RUN_MANUALLY: glyph(Zap),
  SCHEDULED_TIME: glyph(Timer),
  TASK_TYPE_CHANGED: glyph(Shapes),
  TASK_TITLE_CHANGED: glyph(Type),
  TASK_DESCRIPTION_CHANGED: glyph(AlignLeft),
  TASK_ESTIMATE_CHANGED: glyph(Hash),
  TASK_DUE_DATE_CHANGED: glyph(CalendarClock),
  DUE_DATE_APPROACHING: glyph(CalendarClock),
  TASK_OVERDUE: glyph(CalendarOff),
  TASK_START_DATE_CHANGED: glyph(CalendarPlus),
  START_DATE_APPROACHING: glyph(CalendarPlus),
  START_DATE_PASSED: glyph(CalendarOff),
  APPROVAL_STATUS_CHANGED: glyph(ShieldCheck),
  TASK_NO_LONGER_BLOCKED: glyph(CircleSlash),
  ATTACHMENT_ADDED: glyph(Paperclip),
  COLLABORATOR_ADDED: glyph(Users),
  // Generated per field, so keyed by the half before the colon — the same
  // fallback the condition side's `customField` uses.
  customFieldTrigger: glyph(SlidersHorizontal),

  /* Actions the engine performs. */
  ASSIGN_USER: glyph(UserPlus),
  UNASSIGN_USER: glyph(UserMinus),
  MOVE_TO_SECTION: glyph(MoveRight),
  MOVE_TO_PROJECT: glyph(FolderInput),
  UPDATE_STATUS: glyph(CircleDot),
  UPDATE_PRIORITY: glyph(Flag),
  SET_DUE_DATE: glyph(CalendarClock),
  CLEAR_DUE_DATE: glyph(CalendarOff),
  SET_START_DATE: glyph(CalendarPlus),
  CLEAR_START_DATE: glyph(CalendarOff),
  SET_ESTIMATE: glyph(Hash),
  ...
  estimatedMinutes: glyph(Hash),
  createdAt: glyph(Calendar),
  completedAt: glyph(CircleCheck),
  // Generated per field, so keyed by the half before the colon — the same
...
/**
 * The generated rows' glyphs, by what the field holds rather than what it is.
 *
 * Twenty rows all wearing the sliders icon is a column of decoration; the type
 * is the one thing that tells "Started is…" apart from "Status is…" before the
 * words are read, and it is the same signal the field editor uses. Keyed by
 * `ConditionValueType`, which every condition entry carries.
 */
const BY_VALUE_TYPE: Record<string, ReactNode> = {
  SINGLE_SELECT: glyph(CircleChevronDown),
  MULTI_SELECT: glyph(ListChecks),
  DATE: glyph(Calendar),
  TEXT: glyph(Type),
  NUMBER: glyph(Hash),
  CHECKBOX: glyph(CircleCheck),
  PEOPLE: glyph(Users),
};
 ...
 * not like something the map has never heard of. A generated condition row
 * shows its field's own type instead of the generic sliders, when the entry
 * says what that type is.
 */
export function catalogueIcon(subtype: string, valueType?: string): ReactNode {
  if (subtype.startsWith('customField:') && valueType && BY_VALUE_TYPE[valueType]) {
    return BY_VALUE_TYPE[valueType];
  }
